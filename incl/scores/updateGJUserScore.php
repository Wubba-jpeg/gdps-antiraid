...
if(empty($_POST["udid"]) AND empty($_POST["accountID"]))
	exit("-1");
$query = $db->prepare("SELECT COALESCE(SUM(starStars), 0) FROM levels WHERE starStars > 0");
$query->execute();
$levelStars = $query->fetchColumn();
// maxstars
$query = $db->prepare("SELECT COALESCE(SUM(stars), 0) FROM mappacks");
$query->execute();
$mapPackStars = $query->fetchColumn();
$maxstars = $levelStars + $mapPackStars + $starsbuffer;
